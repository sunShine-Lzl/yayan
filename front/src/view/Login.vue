<template>
  <div id="login">
    <div
      class="header animate__animated animate__bounceInRight animate__delay-1s	 "
      v-if="isshow"
    >
      <span>
        欢迎登录雅颜后台管理系统
    ...
    submitForm(formName) {
      this.$refs[formName].validate(async (valid) => {
        ...
        if (valid) {
          ...
          console.log(res);
          if (res.data.code === 0) {
            //登录验证成功后获得该权限的路由表信息动态添加到路由表中

            this.$message.success("登录成功");
            sessionStorage.setItem("token", res.data.token);
            sessionStorage.setItem("uid", res.data.uid);
            console.log(data1);
            this.$router.addRoutes(data1);
            // sessionStorage.setItem("admin", this.ruleForm.user);
